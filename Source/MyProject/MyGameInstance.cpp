...
void UMyGameInstance::Init()
{
	...
	UClass* ClassRunTime = GetClass(); // UClass 포인터 얻기
	UClass* ClassCompile = UMyGameInstance::StaticClass(); // StaticClass()로 UClass 포인터 얻기
	//check(ClassRunTime != ClassCompile);  // 두 포인터가 같은지 확인
	//ensure(ClassRunTime != ClassCompile);  // 두 포인터가 같은지 확인(릴리즈 빌드에서도 동작)
	//ensureMsgf(ClassRunTime != ClassCompile, TEXT("에러 발생")); // 두 포인터가 같은지 확인, 메시지 출력


	UE_LOG(LogTemp, Log, TEXT("학교를 담당하는 클래스 이름: %s"), *ClassRunTime->GetName());

	SchoolName = TEXT("대학교");  // 학교 이름 변경
	// 기존 학교 이름은 템플릿에 저장되고, 변경된 학교 이름은 인스턴스에 저장됨
